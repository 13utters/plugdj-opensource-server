...
module.exports = function (getMain, data, con, res) {
    ...
    function postShuffle() {
        ...
        if (!(playlist)) {
            res.ends(['Playlist notFound (unlike plug, ids are per user)'], 404, 'notFound');
            return;
        } 
    ...
    function postDeleteMedia() {
        ...
        if (!(data.ids)) {
            res.ends(['Missing ids field'], 400, 'badRequest');
            return;
        }
    ...
    function putMoveMedia() {
        ...
        if (!((typeof bid) === 'number')) {
            res.ends(['Missing bid'], 400, 'badRequest');
            return;
        }
        
        if (!((typeof id) === 'number')) {
            res.ends(['Missing id'], 400, 'badRequest');
            return;
        }
        ...
        if (!(item)) {
            res.ends(['Item notFound (unlike plug, ids are per user)'], 404, 'notFound');
            return;
        } 
    ...
    function putMediaUpdate() {
        ...
        if (!(media)) {
            res.ends(['Media notFound (unlike plug, ids are per user)'], 404, 'notFound');
            return;
        }
    ...
    function deletePlaylist() {
        ...
        store.playlists.removeEntry(playlist);
    ...
    function postRename() {
        ...
        if (!(data.name)) {
            res.ends(['Missing name'], 400, 'badRequest');
            return;
        }
        playlist.data = name;
        res.ends(playlist);
