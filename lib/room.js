...
Room.prototype.userById = function (id) {
    var _self = this;
...
Room.prototype.getRole = function (userId) {
    ...
    return match ? match.role : !user ? 0 : user.store().gRole > 4 ? 6 : 0;
};
...
Room.prototype.stopPlaying = function (justStarted, skipped, success) {
    ...
    if (success) {
        if (this.iterateDJs) {
            ...
            if (back) {
                this.backlist.removeEntry(back);
                this.start(back);
                return
            }
        } else if (this.usersId.contains(this.playing.dj.id)) {
...
//Not yet implemented in regular plug
...
Room.prototype.sys = function (message) {
    var _self = this;
    this.users().forEach(function (user) {
        user.socket.sendEvent('plugMessage', message);
    });
};
...
Room.prototype.getSafeUsers = function () {
    ...
    return this.users().filter(function (session) {
        ...
        return _self.getSafeUser(store, _self.getRole(store.id));
    });
};

Room.prototype.getSafeUser = function (store, role) {
    return {
...
Room.prototype.getSPayload = function (store) {
    var _self = this;
    var titles =  _self.playing.media.title;
    var videodir = 'https://i.ytimg.com/vi/' + _self.playing.media.cid + '/hqdefault.jpg';
    var mediadir = _self.playing.media.title + ' - ' +_self.playing.media.author;

    if(titles=="Nothing Playing Right Now")
    {
        videodir = 'http://i.imgur.com/UB7qT3y.jpg';
        mediadir = "Nothing Playing Right Now";
    }
